...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dd = "UPDATE `departments` SET `dept_name`='$dept_name',`status`='$status' WHERE `id`='$id'";
    $cc = mysqli_query($con, $dd);

    // Check if the insert was successful and return the appropriate response
    if ($cc) {
        http_response_code(200);
        echo json_encode(["message" => "Department updated successfully", "code" => 200]);
    } else {
        http_response_code(500);
        echo json_encode(["error" => "Failed to update Department", "code" => 500]);
    }
} else {
